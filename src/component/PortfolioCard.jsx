...
const PortfolioCard = ({imageURL, cardName, cardDescription, cardLink}) => {
  return (
    <div className="flex flex-col w-full justify-between items-center gap-5 p-5 rounded-3xl border border-border bg-cardBg shadow-default xxs:rounded-2xl xxs:p-4 xxs:gap-4 xs:gap-6 xs:p-5 left-[100%]" style={{ aspectRatio: '14/10' }}>
      ...
      <div className="w-full flex flex-row justify-between">
        <div className="flex flex-col gap-2 lg:gap-1 xxs:gap-1">
            <p className="text-2xl font-semibold 2xl:text-[28px] lg:text-xl xxs:text-xl xs:text-[22px]">{cardName}</p>
            <p className="text-lg font-bodyfont text-textDescription 2xl:text-xl lg:text-[16px] xxs:text-[16px] xs:text-lg">{cardDescription}</p>
        </div>
        <div className="flex justify-end items-end ">
            <Link className="border border-border rounded-lg" to={cardLink}>
                <MdArrowOutward className="w-8 h-8 p-2 2xl:w-10 2xl:h-10 xs:h-9 xs:w-9" />
            </Link>
        </div>
      </div>
    </div>
